...
// Deposits
$stmt = $mysqli->prepare("SELECT SUM(transaction_amt) FROM ib_transactions WHERE client_id = ? AND tr_type = 'Deposit'");
$stmt->bind_param('i', $client_id);
$stmt->execute();
$stmt->bind_result($iB_deposits);
$stmt->fetch();
$stmt->close();
$iB_deposits = $iB_deposits ?? 0;

// Withdrawals
$stmt = $mysqli->prepare("SELECT ABS(SUM(transaction_amt)) FROM ib_transactions WHERE client_id = ? AND tr_type = 'Withdrawal'");
$stmt->bind_param('i', $client_id);
$stmt->execute();
...
// Transfers Sent
$stmt = $mysqli->prepare("SELECT SUM(transaction_amt) FROM ib_transactions WHERE client_id = ? AND tr_type = 'Transfer'");
$stmt->bind_param('i', $client_id);
$stmt->execute();
$stmt->bind_result($iB_TransfersSent);
$stmt->fetch();
$stmt->close();
$iB_TransfersSent = $iB_TransfersSent ?? 0;
...
$stmt = $mysqli->prepare("
  ...
  WHERE tr_type = 'Deposit'
    AND receiving_acc_no IS NOT NULL
    AND client_id != ?
    AND account_number IN (
      SELECT account_number FROM ib_bankaccounts WHERE client_id = ?
    )
");
$stmt->bind_param('ii', $client_id, $client_id);
$stmt->execute();
$stmt->bind_result($iB_TransfersReceived);
$stmt->fetch();
$stmt->close();
$iB_TransfersReceived = $iB_TransfersReceived ?? 0;

// ✅ Final Wallet Balance
$TotalBalInAccount = $iB_deposits + $iB_TransfersReceived - ($iB_withdrawal + $iB_TransfersSent);

// Optional: For footer display
            ...
            <div class="col-12 col-sm-6 col-md-3">
              <div class="info-box mb-3">
                ...
                <div class="info-box-content">
                  ...
                  <span class="info-box-number">৳ <?php echo $iB_TransfersSent; ?> </span>
                </div>
              </div>
            </div>
            <!-- /.Transfers-->
              ...
              <div class="card">
                ...
                <div class="card-footer">
                  <div class="row">
                    ...
                    <div class="col-sm-3 col-6">
                      <div class="description-block border-right">
                        <h5 class="description-header">৳ <?php echo $iB_TransfersSent; ?> </h5>
...
window.onload = function () {
  ...
  var Piechart = new CanvasJS.Chart("PieChart", {
    ...
    data: [{
      type: "pie",
      showInLegend: true,
      toolTipContent: "{name}: <strong>{y}</strong> Accounts",
      indexLabel: "{name} - {y}",
      dataPoints: [
        {
          y: <?php
  ...
  var AccChart = new CanvasJS.Chart("AccountsPerAccountCategories", {
    ...
    data: [{
      ...
      dataPoints: [
        {
          y: <?php
              $result = "SELECT ABS(SUM(transaction_amt)) FROM ib_transactions WHERE tr_type = 'Withdrawal' AND client_id = ?";
              $stmt = $mysqli->prepare($result);
              $stmt->bind_param('i', $client_id);
              $stmt->execute();
              $stmt->bind_result($withdrawals_amt);
              $stmt->fetch();
              $stmt->close();
              echo $withdrawals_amt ?? 0;
              ?>,
          name: "Withdraw"
        },
        {
          y: <?php
              $result = "SELECT SUM(transaction_amt) FROM ib_transactions WHERE tr_type = 'Deposit' AND client_id = ?";
              $stmt = $mysqli->prepare($result);
              $stmt->bind_param('i', $client_id);
              $stmt->execute();
              $stmt->bind_result($deposits_amt);
              $stmt->fetch();
              $stmt->close();
              echo $deposits_amt ?? 0;
              ?>,
        ...
        {
          y: -<?php
              $result = "SELECT SUM(transaction_amt) FROM ib_transactions WHERE tr_type = 'Transfer' AND client_id = ?";
              $stmt = $mysqli->prepare($result);
              $stmt->bind_param('i', $client_id);
              $stmt->execute();
              $stmt->bind_result($transfers_amt);
              $stmt->fetch();
              $stmt->close();
              echo $transfers_amt ?? 0;
              ?>,
          name: "Transfer"
        }
      ]
