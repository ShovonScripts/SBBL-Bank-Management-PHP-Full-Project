...
if (isset($_POST['deposit'])) {
    ...
    $transaction_amt = floatval($_POST['transaction_amt']);
    ...
    if ($transaction_amt > $amt || $account_number === $receiving_acc_no) {
    $transaction_error  =  "Invalid transfer: either insufficient funds or trying to transfer to the same account.";
    } else {
...
$receiver_stmt = $mysqli->prepare($receiver_query);
$receiver_stmt->bind_param("s", $receiving_acc_no);
$receiver_stmt->execute();
$receiver_stmt->bind_result($receiver_acc_id, $receiver_client_id, $receiver_client_name, $receiver_nid, $receiver_phone, $receiver_acc_type);
$receiver_stmt->fetch();
$receiver_stmt->close();

if ($receiver_acc_id) {
    // Create new deposit for the receiver
    $receiver_tr_type = 'Deposit';
    $receiver_tr_status = 'Success';

    $deposit_query = "INSERT INTO ib_transactions 
        (tr_code, account_id, acc_name, account_number, acc_type, tr_type, tr_status, client_id, client_name, client_national_id, transaction_amt, client_phone) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $deposit_stmt = $mysqli->prepare($deposit_query);
